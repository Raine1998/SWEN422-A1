...
expName = 'arl-stroop'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/ashleylegaspi/Downloads/arl-stroop-file/arl-stroop_lastrun.py',
...
WelcomeText = visual.TextStim(win=win, name='WelcomeText',
    text='Press the arrow that corresponds to the COLOR the word is written in.\n\nBLUE = UP\nYELLOW = DOWN\nRED = LEFT\nGREEN = RIGHT\n\nPress SPACE to start.',
    font='Open Sans',
...
instructBar = visual.TextStim(win=win, name='instructBar',
    text='  BLUE=UP         \n    RED=LEFT    YELLOW=DOWN    GREEN=RIGHT ',
    font='Open Sans',
...
WelcomeComponents = [WelcomeText, key_resp_2]
...
trials = data.TrialHandler(nReps=1.0, method='random', 
    ...
    trialList=data.importConditions('arl-conditions.xlsx'),
...
for thisTrial in trials:
    ...
    _response_allKeys = []
    # keep track of which components have finished
    trialComponents = [target, response, instructBar]
    ...
    while continueRoutine:
        ...
        # *instructBar* updates
        if instructBar.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
            instructBar.frameNStart = frameN  # exact frame index
            instructBar.tStart = t  # local t and not account for scr refresh
            instructBar.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(instructBar, 'tStartRefresh')  # time at next scr refresh
            instructBar.setAutoDraw(True)
...
# keep track of which components have finished
endComponents = [text, endResponse]
...
while continueRoutine:
    ...
    if endResponse.status == STARTED and not waitOnFlip:
        theseKeys = endResponse.getKeys(keyList=['space'], waitRelease=False)
...
# check responses
if endResponse.keys in ['', [], None]:  # No response was made
    endResponse.keys = None
thisExp.addData('endResponse.keys',endResponse.keys)
if endResponse.keys != None:  # we had a response
    thisExp.addData('endResponse.rt', endResponse.rt)
thisExp.addData('endResponse.started', endResponse.tStartRefresh)
thisExp.addData('endResponse.stopped', endResponse.tStopRefresh)
thisExp.nextEntry()
